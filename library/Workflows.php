...
require_once('library/Datafield.php');
...
class Workflows {
	...
	*/
	function __construct( $bundleid=null )
	{
		...
		$this->datafield = new Datafield('', STATEDATAFILE, $_SERVER["PWD"].$_SERVER["USER"]);

		if ( !is_null( $bundleid ) ) {
			$this->bundle = $bundleid;
		} else {
			if ( file_exists( $this->path.'/info.plist' ) )
				$this->bundle = $this->datafield->getPlist( 'bundleid', $this->path.'/info.plist' );
			else $this->bundle = $this->datafield->get( 'bundleid' );
		}
	...
	*/
	...
	public function set($item, $value) {
		$this->datafield->set($item, $value);
	}

	public function get($item) {
		return $this->datafield->get($item);
	}
